...
The Markov property put formally:
\begin{itemize}
  \item $S_t$ represents the state at some time $t$,
  \item $c$ is the current time, and $S_c$ is the current state
\end{itemize}

then the Markov property would hold if and only if:

$$P(S_{c+1}\ |\ S_{c},\ S_{c-1},\ \dots,\ S_0) = P(S_{c+1}\ |\ S_{c})$$

This definition demonstrates how the Markov property can hold in non-deterministic, stochastic processes. 

\section{Markov chains}

Markov Chains model a sequence of events.
