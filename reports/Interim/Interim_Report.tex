...
\begin{align}
  ...
  \therefore v_{\text{max}}(s) &\leftarrow  \max_a \sum_{s',r}p(s',r\ |\ s, a)[r + \gamma v_{\text{max}}(s')] \label{eqn:v-update}
\end{align}
...
Iteration approaches are effective at finding an optimal policy. However, these approaches make extensive use of the dynamics of the MDP. In most real-world problems, the dynamics are unknown or too complex to be represented accurately. Iteration approaches can be adapted to the agent-environment interface \ref{fig:agent-enviroment} by using samples. Samples are captured from the environment where the agent follows a deterministic policy to explore. However, these adapted approaches can require many samples from the environment. Monti-Carlo is another technique that uses samples to emulate the value functions more directly, but these approaches are inherently offline. While offline methods have many advantages, their shortcomings, such as the inability to adapt to changing environments, make them unsuitable for many applications. 
