...
This application is has been written in Python with The model-view-controller architecture. The application has been written with in a Object Oriented fashion with modern software engineering principles in mind\cite{van2008software}. This application uses Pytest to guarantee all all core, complex, and important functionality. I made this choice based upon my experience from my year in industry that testing can be incredibly valuable in many places 100\% coverage or TDD can be counter productive with, diminishing returns for trivial code, increased baggage with refactoring and sometimes the tests become more complex and bug prone than the code they are testing.


% Python challanges + solutions interfaces. wighter perfomrance

\section{Workflow}

This application has been packaged with poetry for dependency management and ease of installation. the code quality is maintained in this repository with a number of strict liners that: enforce a consistent code style, check for static typing errors ensure documentation is written. All files are stored under version control, and with pre-commit hooks all linters and tests are run before code is committed.
