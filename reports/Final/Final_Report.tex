...
Implementing RL algorithms is the core of this project. A number of papers describe the fundamental concepts, such as the bellman equations~\cite{bellman1957}, Markov Decision Processes~\cite{meyn2012markov,sutton2018reinforcement}, Q-learning~\cite{watkins1992q}, and double Q-learning~\cite{doubleQLearning}. The book \say{Reinforcement Learning An Introduction}~\cite{sutton2018reinforcement} casts a wide net across RL covering touching on most topics discussed in this report. Several papers address the technical challenges of RL, including exploration vs exploitation~\cite{rewardEpsilonDecay},  convergence~\cite{watkins1992q, QlearningConvergance}, and hyperparameter tuning~\cite{deepRLChallanges, searchStrategies}. Some papers explore what deep reinforcement learning is~\cite{deepRLOverview} and the challenges with developing it. These challenges include instability~\cite{deepOnVsOffPolicy,sutton2018reinforcement} as well as the difficulty and cost of tuning~\cite{deepRLChallanges, DeepRLCost}. Other papers relate the RL field to psychology and the study of operant conditioning~\cite{shteingart2014reinforcement,staddon2003operant}.

The core of this project is exploring how to explore effectively. Many papers cover the problematic topic of accurately measuring the performance and accounting or avoiding the issue of hyperparameter tuning~\cite{assessingDeepRL, evaluatingRL, parameterFreeExploration}. Other papers cover how to set up an agent for success with its learning rates and initial values~\cite{even2003learning, deathTransfer,decayingLearningRates}. The choice of exploration strategy has the most profound impact on how efficiently a Q-learning agent explores. The Barto-Sutton book~\cite{sutton2018reinforcement} covers many of the standard options. The paper \say{Model-Free Active Exploration in Reinforcement Learning}~\cite{modelFree} introduces a state-of-the-art exploration strategy based around the best policy identification problem~\cite{bestPolicyIdentifaction}. A paper derives the characteristic time measure~\cite{characteristicTime} that is the basis of this strategy. Another paper proves  proves that it is non-convex~\cite{characteristicTimeNonConvex} making it hard to solve and implement directly into a strategy. 

Software engineering is key part of this project; its an approach to software development to create software that is robust. Several papers describe the key principles of software engineering and how to apply them~\cite{softwareEngineringPrinciples,van2008software}. Unit testing is a key part of software engineering a survey demonstrated its strengths and weaknesses~\cite{unitTestingSurvey}. The application uses a variant of the MVC architecture, several papers detail similar architectures~\cite{webMVC, mvvm, gamesMVC}. A paper covers the technical considers of creating machine learning applications in Python~\cite{pythonMachineLearning}. Two papers detail aspects to modern software development: development containers~\cite{developmentContainers} and inline documentation~\cite{pythonAutoDoc}.

In the professional issues chapter~\ref{chap:professional-issues} this report analyses the social and ethical implications of reinforcement learning and the wider machine learning field. Several papers discuss the details of Amazon \say{Rekognition} case study~\cite{facialRecognitionBias, legislatingRekognition,legislatingOG, rekognitionOverview}. A number of papers highlight the social harms that can arise from biased and unbiased ML programs~\cite{AISocialImpactAndAccountability,facialRecognitionBias, anprAccountability}. Numerous papers discuss how ML programs gain biases from their training~\cite{LLMSocialEthicalRisks, AISocialImpactAndAccountability, facialRecognitionBias}. Two papers describe the military uses of RL such as UAVs~\cite{rlMilitaryReviewChina, DRLDrones}. Finally, a few papers discuss the potential opportunities of RL to benefit society and some potential principles that could aid in mitigating these risks~\cite{AIRisksAndPrinciples,deepRLsocietalImpact, ransbotham2017reshaping}. 
