...
In essence, this equation is optimistic as it will never underestimate or explore an action. It also provides a natural balance between exploration and exploitation, Without a discount factor this strategy naturally tapers off exploration over time. Furthermore instead of even exploration like $\varepsilon$-Greedy; UCB avoids redundant exploration in areas that have already been covered. However, UCB is not without hyperparameters. $c$ controls the exploration balance; higher values encourage more exploration.

\section{TODO - Model-Free Best Policy Identification}

Model-Free Best Policy Identification (MF-BPI) is a state-of-the-art method that has demonstrated an improvement in exploration over other exploration strategies such as UCB \cite{modelFree}. 
