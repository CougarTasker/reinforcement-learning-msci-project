...
\begin{abstract}
  ...
  The program will be developed in Python; this is motivated by Python's strong ecosystem of tooling and community for data processing tasks. Performance is essential with machine learning tasks. Python enables the use of low-level libraries that can use the hardware efficiently with high-level APIs. These high-level APIs abstract over unimportant details, increasing productivity. However, Raschka et al. states:
  ...
  For this reason, The project will start using PyPy. PyPy is a just-in-time (JIT) compiler that runs code four times faster according to PyPy's benchmarks\cite{pypy2023Benchmarks}. Nevertheless, if the project runs into compatibility issues, the project may fall back to CPython. 
  
  To focus the development on the program's main objectives, the application will use the libraries Kivy\cite{kivy} and PyTorch\cite{pytorch}. Kivy is a library that provides functionality to create natural user interfaces quickly\cite{virbel2011kivy}.  

  using the libraries Kivy for graphics and PyTorch\cite{pytorch} for Deep Neural networks. Python 
\end{abstract}
\newpage
